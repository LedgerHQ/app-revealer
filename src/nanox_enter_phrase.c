...
void screen_onboarding_4_restore_word_display_word_selection(void) {
  ...
  G_ux.stack[0].element_arrays[0].element_array = screen_onboarding_4_restore_word_select_elements;
  ...
  switch(event) {
    case KEYBOARD_ITEM_VALIDATED:
      ...
      if (G_ux.string_buffer[32+value] == '\b') {
        ...
        nb_words_matching_stem = bolos_ux_bip39_get_word_count_starting_with((unsigned char*)G_ux.string_buffer+16, strlen(G_ux.string_buffer+16));
...
const bagl_element_t* screen_onboarding_4_restore_word_before_element_display_callback(const bagl_element_t* element) {
  switch(element->component.userid) {
    ...
    case 0x31:
      ...
      break;
    case 0x32:
      if (G_bolos_ux_context.onboarding_step == 0 || G_bolos_ux_context.onboarding_step == G_bolos_ux_context.onboarding_kind-1) {
        return NULL; // don't display
      }
      break;
    case 0x33:
...
unsigned int screen_onboarding_4_restore_word_intro_button(unsigned int button_mask, unsigned int button_mask_counter) {
  UNUSED(button_mask_counter);
  switch(button_mask) {
    case BUTTON_EVT_RELEASED|BUTTON_LEFT|BUTTON_RIGHT: {
      screen_onboarding_4_restore_word_display_auto_complete();
...
void screen_onboarding_4_restore_word_init(unsigned int firstWord) {
  ...
  G_ux.stack[0].button_push_callback = screen_onboarding_4_restore_word_intro_button;
  G_ux.stack[0].screen_before_element_display_callback = screen_onboarding_4_restore_word_before_element_display_callback;

  // elements to be displayed
  G_ux.stack[0].element_arrays[0].element_array = screen_onboarding_4_restore_word_intro_elements;
  G_ux.stack[0].element_arrays[0].element_array_count = ARRAYLEN(screen_onboarding_4_restore_word_intro_elements);
  G_ux.stack[0].element_arrays_count = 1;
  ux_stack_display(0);
